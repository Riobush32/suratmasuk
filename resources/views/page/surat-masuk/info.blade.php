        ...
        <div class="border-gray-300 border text-gray-300 p-5 rounded-xl  max-w-[1100px]">
            ...
            <div class="grid grid-cols-2 gap-4 mt-3">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="text-gray-400">
                        <h1>Instansi Pengirim </h1>
                    </div>
                    <div class="text-white">
                        : <span>{{ $suratMasuk->instansi_pengirim }}</span>
                    </div>
                    <div class="text-gray-400">
                        <h1>Nomor Agenda </h1>
                    </div>
                ...
                <div class="">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="text-white">
                            : <span>{{ $suratMasuk->tanggal_surat }}</span>
                        </div>
                        <div class="text-gray-400">
                            <h1>Tanggal Diterima </h1>
                        </div>
                        <div class="text-white">
                            : <span>{{ $suratMasuk->tanggal_diterima }}</span>
                        </div>
                        <div class="text-gray-400">
                            <h1>klasifikasi </h1>
                        </div>
                        <div class="text-white">
                            : <div class="badge bg-{{ $suratMasuk->klasifikasi->color_list->color }}">
                                <span class="text-white text-light">{{ $suratMasuk->klasifikasi->name }}</span>
                            </div>
                        </div>
                        <div class="text-gray-400">
                            <h1>Status </h1>
                        ...
                        <div class="text-white">
                            : <div class="badge bg-{{ $suratMasuk->status->color_list->color }}">
                                <span class="text-white text-light">{{ $suratMasuk->status->name }}</span>
                            </div>
                        </div>
                        <div class="text-gray-400">
                            <h1>Sifat </h1>
                        ...
                        <div class="text-white">
                            : <div class="badge bg-{{ $suratMasuk->sifat->color_list->color }}">
                                <span class="text-white text-light">{{ $suratMasuk->sifat->name }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            @if ($suratMasuk->file_patch != '')
                <div class="my-3">
                    <a href="{{ asset('suratmasuks/' . $suratMasuk->file_patch) }}" target="_blank"
                        class="btn btn-ghost btn-xs mx-1 text-lime-300 font-light">
                        <i class="fa-regular fa-eye"></i>
                        {{ $suratMasuk->file_patch }}
                    </a>
                </div>
            @endif
            ...
            <div class="w-60">
                ...
                <div contenteditable="true"
                    ...
                    @forelse ($catatan as $catat)
                        <h6>
                            <span class="{{ $catat->user->role == 'staff' ? 'text-blue-300' : ($catat->user->role == 'sekertaris' ? 'text-rose-300' : 'lime-300') }}">
                                {{ $catat->user->role }} :
                            </span>
            ...
            <form action="{{ route('infoUpdate') }}" method="POST" class="w-full flex justify-between items-end">
                @csrf @method('patch')
                ...
                <div class="flex items-start gap-4">
                    <label class="text-black form-control w-full max-w-xs">
                        ...
                        <select
                            ...
                            <option disabled selected value="{{ $suratMasuk->status->id }}">
                                {{ $suratMasuk->status->name }}</option>
                            @if ($suratMasuk->status->name == 'diperiksa staff')
                                <option value="2">Ajukan Ke Sekertaris</option>
                            @endif
                            @if ($suratMasuk->status->name == 'diperiksa sekertaris')
                                <option value="1">Ajukan Ke Kepala</option>
                                <option value="3">Koreksi Kembali</option>
                            @endif
                            @if ($suratMasuk->status->name == 'diperiksa kepala')
