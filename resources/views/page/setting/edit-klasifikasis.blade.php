<x-dashboard>
    ...
    <div class="w-full flex justify-center">
        <div class="border-gray-300 border text-gray-300 p-5 rounded-xl  max-w-[1100px]">
            <div class="">
                <legend class="text-2xl">Edit Klasifikasi</legend>
                <hr class="my-2">
            </div>
            <form action="{{ route('klasifikasiUpdate') }}" method="POST" class="" >
                @csrf @method('patch')
                <input type="hidden" value="{{ $klasifikasi->id }}" name="id">
                <div class="flex flex-wrap gap-4">
                    <label class="text-black form-control w-full max-w-xs">
                        <div class="label">
                            <span class="label-text text-gray-50">Nama</span>
                        </div>
                        <input type="text" name="name" value="{{ $klasifikasi->name }}" id="nomor_serat"
