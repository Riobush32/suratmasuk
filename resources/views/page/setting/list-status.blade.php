<div class="mt-5  p-5 rounded-xl">
    <div class="overflow-x-auto" x-data="{
        ...
        modalData: null,
        getDeleteUrl() {
            return `{{ route('statusDestroy', '') }}/${this.modalData}`;
        }
    }">
        <div class="w-full flex justify-between items-center p-3">
            <div class="text-primary font-light">Status Surat</div>
            <div class="">
                <a href="{{ route('statusAdd') }}" class="btn btn-primary btn-sm">Add New</a>
            </div>
        </div>
        <table class="table">
            <!-- head -->
            ...
            <tbody>
                ...
                @forelse ($statuses as $item)
                    <tr>
                        ...
                        <th class="text-end">
                            <a href="{{ route('statusEdit', ['id' => $item->id]) }}"
