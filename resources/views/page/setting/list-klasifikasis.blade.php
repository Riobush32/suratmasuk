<div class="mt-5  p-5 rounded-xl">
    <div class="overflow-x-auto" x-data="{
        ...
        modalData: null,
        getDeleteUrl() {
            return `{{ route('klasifikasiDestroy', '') }}/${this.modalData}`;
        }
    }">
        <div class="w-full flex justify-between items-center p-3">
            <div class="text-primary font-light">Klasifikasi</div>
            <div class="">
                <a href="{{ route('addKlasifikasi') }}" class="btn btn-primary btn-sm">Add New</a>
            </div>
        </div>
        <table class="table">
            <!-- head -->
            ...
            <tbody>
                ...
                @forelse ($klasifikasis as $item)
                    <tr>
                        ...
                        <th class="text-end">
                            <a href="{{ route('klasifikasiEdit', ['id' => $item->id]) }}"
